...
from tracepointdebug.broker.request.filter_logpoints_request import (
    FilterLogPointsRequest,
)
from tracepointdebug.broker.request.filter_tracepoints_request import (
...
class BrokerManager(object):
    ...
    def __init__(self):
        ...
        if sys.version_info[0] >= 3:
            self.application_status_thread = Thread(
                target=self.application_status_sender, daemon=True
            )
        else:
            self.application_status_thread = Thread(
                target=self.application_status_sender
            )
    ...
    def create_request():
        application_info = Application.get_application_info()
        filter_tracepoints_request = FilterTracePointsRequest(
        ...
        filter_logpoints_request = FilterLogPointsRequest(
            application_info.get("applicationName", ""),
            application_info.get("applicationVersion", ""),
            application_info.get("applicationStage", ""),
            application_info.get("applicationTags", {}),
    ...
    def application_status_sender(self):
        while (
            self.broker_connection is not None and self.broker_connection.is_running()
        ):
