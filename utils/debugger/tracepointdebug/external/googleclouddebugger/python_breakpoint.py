...
from . import imphook2, module_explorer, module_search2, module_utils2
...
ERROR_LOCATION_FILE_EXTENSION_0 = "Only files with .py extension are supported"
...
class PythonBreakpoint(object):
    ...
    def _ActivateBreakpoint(self, module):
            ...
            self._CompleteBreakpoint(
                {
                    "status": {
                        "isError": True,
                        "refersTo": "BREAKPOINT_SOURCE_LOCATION",
                        "description": {"format": fmt, "parameters": params},
                    }
                }
            )
            return
        ...
        self._cookie = native.SetConditionalBreakpoint(
            codeobj, line, condition, self._BreakpointEvent
        )
    ...
    def _BreakpointEvent(self, event, frame):
        ...
        try:
            ...
            error_status = {
                "isError": True,
                "description": {"format": "Unknown internal error"},
            }
            self._CompleteBreakpoint({"status": error_status})
            return
