...
class CaptureCollector(object):
    ...
    def Collect(self, top_frame):
        ...
        try:
            ...
            self.breakpoint["status"] = {
                ...
                "description": {
                    "format": (
                        "INTERNAL ERROR: Failed while capturing locals "
                        "of frame $0: $1"
                    ),
    ...
    def CaptureNamedVariable(self, name, value, depth, limits):
        ...
        v = self.CheckDataVisibility(value) or self.CaptureVariable(
            value, depth, limits
        )
        v["name"] = name
        return v
    ...
    def CaptureVariableInternal(self, value, depth, limits, can_enqueue=True):
        ...
        if isinstance(value, dict):
            ...
            return {
                "members": self.CaptureVariablesList(
                    items, depth + 1, EMPTY_DICTIONARY, limits
                ),
                "type": "dict",
            }
        ...
        for pretty_printer in CaptureCollector.pretty_printers:
            ...
            return {
                "members": self.CaptureVariablesList(
                    fields, depth + 1, OBJECT_HAS_NO_FIELDS, limits
                ),
                "type": object_type,
            }
        ...
        members = self.CaptureVariablesList(
            items, depth + 2, OBJECT_HAS_NO_FIELDS, limits
        )
        v = {"members": members}
    ...
    def _CaptureExpression(self, frame, expression):
        ...
        return self.CaptureNamedVariable(
            expression, value, 0, self.expression_capture_limits
        )
...
class LogCollector(object):
    ...
    def Log(self, frame):
        ...
        cdbg_logging_location = (
            NormalizePath(frame.f_code.co_filename),
            line,
    ...
    def _FormatExpression(self, frame, expression):
        ...
        if not rc:
            message = _FormatMessage(
                value["description"]["format"], value["description"].get("parameters")
            )
            return "<" + message + ">"
    ...
    def _FormatValue(self, value, level=0):
        ...
        if isinstance(value, _PRIMITIVE_TYPES):
            return _TrimString(
                repr(value), self.max_value_len  # Primitive type, always immutable.
            )
    ...
    try:
        ...
        return (
            False,
            {
                "isError": True,
                "refersTo": "VARIABLE_NAME",
                "description": {"format": "Invalid expression", "parameters": [str(e)]},
            },
        )
