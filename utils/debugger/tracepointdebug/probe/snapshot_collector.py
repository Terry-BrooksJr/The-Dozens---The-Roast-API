...
class SnapshotCollector(object):
    ...
    def collect(self, top_frame):
        ...
        while frame and len(collected_frames) < self.max_frames:
            ...
            if len(collected_frames) < self.max_expand_frames:
                collected_frames.append(
                    Frame(
                        frame.f_lineno,
                        self.collect_frame_locals(frame=frame),
                        file_path,
                        code.co_name,
                    )
                )
                ...
                collected_frames.append(
                    Frame(frame.f_lineno, Variables([]), file_path, code.co_name)
                )
            frame = frame.f_back
        ...
        snapshot = Snapshot(
            frames=collected_frames, method_name=top_frame_method_name, file=file
        )
        return snapshot
