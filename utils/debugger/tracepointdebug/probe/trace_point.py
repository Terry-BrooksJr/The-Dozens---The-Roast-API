...
from tracepointdebug.external.googleclouddebugger.module_explorer import (
    GetCodeObjectAtLine,
)
...
from tracepointdebug.probe.errors import (
    ...
    LINE_NO_IS_NOT_AVAILABLE,
    LINE_NO_IS_NOT_AVAILABLE_2,
    LINE_NO_IS_NOT_AVAILABLE_3,
    PUT_TRACEPOINT_FAILED,
    SOURCE_CODE_MISMATCH_DETECTED,
)
from tracepointdebug.probe.event.put_tracepoint_failed_event import (
    PutTracePointFailedEvent,
)
from tracepointdebug.probe.event.trace_point_rate_limit_event import (
    TracePointRateLimitEvent,
)
from tracepointdebug.probe.event.trace_point_snapshot_event import (
    TracePointSnapshotEvent,
)
from tracepointdebug.probe.event.tracepoint_snapshot_failed_event import (
...
class TracePoint(object):
    def __init__(self, trace_point_manager, trace_point_config):
        ...
        if os.path.splitext(self.config.file)[1] != ".py":
            raise CodedException(
                PUT_TRACEPOINT_FAILED,
                (
                    self.config.get_file_name(),
                    self.config.line,
                    self.config.client,
                    "Only .py file extension is supported",
                ),
            )

        if trace_point_config.expire_duration != -1:
            self.timer = Timer(
                trace_point_config.expire_duration,
                self.trace_point_manager.expire_trace_point,
                args=(self,),
            ).start()
        ...
        # Add an import hook to later set the trace point
            ...
            self._import_hook_cleanup = imphook2.AddImportCallbackBySuffix(
                source_path, self.set_active_trace_point
            )
        ...
        try:
            ...
            if self.config.cond:
                try:
                    ...
                    self.condition = ConditionFactory.create_condition_from_expression(
                        self.config.cond
                    )
                except Exception as e:
                    raise CodedException(
                        CONDITION_CHECK_FAILED, (self.config.cond, str(e))
                    )

            logger.info(
                "Creating new Python breakpoint %s in %s, line %d"
                % (self.id, code_object, self.config.line)
            )
            ...
            self._cookie = cdbg_native.SetConditionalBreakpoint(
                code_object, self.config.line, None, self.breakpoint_callback
            )
        except Exception as exc:
            code = 0
            if isinstance(exc, CodedException):
                code = exc.code
            event = PutTracePointFailedEvent(
                self.config.get_file_name(), self.config.line, code, str(exc)
            )
    ...
    def breakpoint_callback(self, event, frame):
        try:
            ...
            self.hit_count += 1
            if (
                self.config.expire_hit_count != -1
                and self.hit_count >= self.config.expire_hit_count
            ):
            ...
            if rate_limit_result == RateLimitResult.HIT:
                event = TracePointRateLimitEvent(
                    self.config.get_file_name(), self.config.line
                )
                event.client = self.config.client
                self.trace_point_manager.publish_event(event)
            ...
            snapshot_collector = SnapshotCollector(
                _MAX_SNAPSHOT_SIZE, _MAX_FRAMES, _MAX_EXPAND_FRAMES
            )
            ...
            transaction_id = (
                None if not trace_context else trace_context.get_transaction_id()
            )
            span_id = None if not trace_context else trace_context.get_span_id()

            event = TracePointSnapshotEvent(
                self.id,
                self.config.get_file_name(),
                self.config.line,
            ...
            if isinstance(exc, CodedException):
                code = exc.code
            event = TracePointSnapshotFailedEvent(
                self.config.get_file_name(), self.config.line, code, str(exc)
            )
            event.client = self.config.client
            self.trace_point_manager.publish_event(event)
