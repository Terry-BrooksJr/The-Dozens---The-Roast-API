...
from tracepointdebug.probe.request.logPoint.put_log_point_request import (
    PutLogPointRequest,
)
from tracepointdebug.probe.response.logPoint.put_log_point_response import (
    PutLogPointResponse,
)
...
class PutLogPointRequestHandler(RequestHandler):
    ...
    def handle_request(request):
        ...
        try:
            ...
            log_point_manager.put_log_point(
                request.log_point_id,
                request.file,
                request.file_hash,
                request.line_no,
                request.get_client(),
                request.expire_secs,
                request.expire_count,
                False,
                request.log_expression,
                request.condition,
                request.log_level,
                request.stdout_enabled,
            )
