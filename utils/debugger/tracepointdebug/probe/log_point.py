...
from tracepointdebug.probe.event.logpoint.log_point_failed_event import (
    LogPointFailedEvent,
)
from tracepointdebug.probe.event.logpoint.put_logpoint_failed_event import (
    PutLogPointFailedEvent,
)
...
class LogPoint(object):
    def __init__(self, log_point_manager, log_point_config):
        ...
        if log_point_config.expire_duration != -1:
            self.timer = Timer(
                log_point_config.expire_duration,
                self.log_point_manager.expire_log_point,
                args=(self,),
            ).start()
        ...
        # Add an import hook to later set the log point
            ...
            self._import_hook_cleanup = imphook2.AddImportCallbackBySuffix(
                source_path, self.set_active_log_point
            )
        ...
        try:
            ...
            if self.config.file_hash:
                ...
                if source_hash and source_hash != self.config.file_hash:
                    raise CodedException(
                        errors.SOURCE_CODE_MISMATCH_DETECTED,
                        (
                            "logpoint",
                            self.config.get_file_name(),
                            self.config.line,
            ...
            if self.config.cond:
                try:
                    ...
                    raise CodedException(
                        errors.CONDITION_CHECK_FAILED, (self.config.cond, str(e))
                    )
