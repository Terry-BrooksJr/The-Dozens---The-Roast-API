...
class LogPointManager(object):
    ...
    def instance(*args, **kwargs):
        return (
            LogPointManager(*args, **kwargs)
            if LogPointManager.__instance is None
            else LogPointManager.__instance
        )
        ...
        with self._lock:
            if log_point_id not in self._log_points:
                raise CodedException(
                    errors.NO_LOGPOINT_EXIST_WITH_ID, (log_point_id, client)
                )
            log_point = self._log_points.pop(log_point_id)
            log_point.remove_log_point()
            log_point_config = LogPointConfig(
                log_point_id,
                log_point.config.file,
                log_point.config.file_ref,
                log_point.config.line,
                client,
                log_expression,
                condition,
                expire_duration,
    ...
    def put_log_point(
        ...
        with self._lock:
            if log_point_id in self._log_points:
                raise CodedException(
                    errors.LOGPOINT_ALREADY_EXIST, (file, line, client)
                )
