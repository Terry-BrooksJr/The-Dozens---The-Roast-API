...
class TracePointManager(object):
    ...
    def instance(*args, **kwargs):
        return (
            TracePointManager(*args, **kwargs)
            if TracePointManager.__instance is None
            else TracePointManager.__instance
        )
        ...
        with self._lock:
            if trace_point_id not in self._trace_points:
                raise CodedException(
                    errors.NO_TRACEPOINT_EXIST_WITH_ID, (trace_point_id, client)
                )
            trace_point = self._trace_points.pop(trace_point_id)
            trace_point.remove_trace_point()
            trace_point_config = TracePointConfig(
                trace_point_id,
                trace_point.config.file,
                trace_point.config.file_ref,
                trace_point.config.line,
                client,
                condition,
                expire_duration,
                expire_count,
    ...
    def put_trace_point(
        ...
        with self._lock:
            if trace_point_id in self._trace_points:
                raise CodedException(
                    errors.TRACEPOINT_ALREADY_EXIST, (file, line, client)
                )
            file, file_ref = file.split(
                "?ref="
            )  # Commit id has been added to fileName. To get actual path, remove commit id.
            trace_point_config = TracePointConfig(
                trace_point_id,
                file,
                file_ref,
